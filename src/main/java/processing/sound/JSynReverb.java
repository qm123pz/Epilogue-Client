...
class JSynReverb extends UnitFilter {
	...
	public JSynReverb() {
		...
		processing.sound.JSynAllPass first = new processing.sound.JSynAllPass(0.5, JSynReverb.As[0]);
		this.reverbCircuit.add(first);
		processing.sound.JSynAllPass ap = first;
		for (int i = 1; i < JSynReverb.As.length; i++) {
			processing.sound.JSynAllPass next = new processing.sound.JSynAllPass(0.5, JSynReverb.As[i]);
