...
/**
 * This is a Soundfile player which allows to play back and manipulate processing.sound
 ...
 **/
public class MultipleSoundFile extends AudioSample {
	...
	/**
	 * @param path
	 *            filename of the processing.sound file to be loaded
	 */
	@SneakyThrows
	public MultipleSoundFile(List<File> files) {
		super();

		List<FloatSample> loadedSamples = new ArrayList<>();

		for (File file : files) {
			loadedSamples.add(SampleLoader.loadFloatSample(file));
        }

		this.sample = new MultipleFloatSample(loadedSamples.toArray(new FloatSample[0]));
	...
	/**
	 * Move the processing.sound in a stereo panorama.-1.0 pans to the left channel and 1.0 to the
	 ...
	 *            (right).
	 * @webref Sampling:SoundFile
	 * @webBrief Move the processing.sound in a stereo panorama.
	 **/
