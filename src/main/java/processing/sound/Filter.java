...
public abstract class Filter<E extends FilterBiquadCommon> extends Effect<E> {
	...
	public void amp(float amp) {
		this.left.amplitude.set(amp);
		this.right.amplitude.set(amp);
	}
 ...
 /**
	...
	*/
	public void res(Modulator modulator) {
		processing.sound.Engine.setModulation(this.left.Q, modulator);
		processing.sound.Engine.setModulation(this.right.Q, modulator);
	}
 ...
 /**
	...
	*/
	public void freq(Modulator modulator) {
		processing.sound.Engine.setModulation(this.left.frequency, modulator);
		processing.sound.Engine.setModulation(this.right.frequency, modulator);
	}
	
	public void process(processing.sound.SoundObject input, float freq) {
		this.freq(freq);
		this.process(input);
	}
	 ...
	 * @param input the processing.sound source to filter
	 * @param freq the cutoff frequency in Hertz
