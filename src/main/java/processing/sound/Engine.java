...
public class Engine {
	 ...
	 * library run on several different processing.sound devices simultaneously, by first
	...
	protected static boolean checkAmp(float amp) {
		if (amp < -1 || amp > 1) {
			...
			Engine.printWarning("an amplitude of 0 means this processing.sound is not audible now");
		}
		return true;
	}
