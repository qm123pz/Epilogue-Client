...
public class KJFFT {
	...
	public KJFFT( int pSampleSize ) {
		this.init(pSampleSize);
	}

    public void init( int pSampleSize ) {
        ss  = pSampleSize;
        ss2 = ss >> 1;
     ...
     */
    public float[] calculate( float[] pSample, float[] pImaginaryPart ) {

        if (ss != pSample.length) {
            this.init(pSample.length);
        }
        ...
        //           processing.sound we cannot hear anyway.
